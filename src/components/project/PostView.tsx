...
export default function PostsSection({
  ...
  // مكون عرض منشور واحد
  const SinglePostView = ({ post, currentUserId, currentUserName, onPostUpdate, onPostDelete }: SinglePostViewProps) => {
    return (
      <div className="space-y-6">
        {/* زر العودة إلى المشروع */}
        <div className="mb-6">
          <Link
            href={`/projects/${post.project_id}`}
            className="inline-flex items-center px-4 py-2 rounded-md transition-colors rtl bg-indigo-600 text-white hover:bg-indigo-700"
          >
                ...
                <div className="mr-3 flex-1">
                  ...
                  {showCommentCodeInput === post.id && (
                    <div className="mt-2">
                      <div className="flex items-center mb-2">
                        <label className="text-sm text-gray-600 ml-2">لغة البرمجة:</label>
                        <select
                      ...
                      <textarea
                        ...
                        placeholder="أضف كود هنا..."
                        className="w-full p-3 border border-gray-300 rounded-md font-mono text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        rows={5}
                      />
                    </div>
                  )}
                  ...
                  <div className="mt-2 flex justify-between">
                    <div>
                      <button
                        type="button"
                        onClick={() => setShowCommentCodeInput(showCommentCodeInput === post.id ? null : post.id)}
                        className="text-indigo-600 hover:text-indigo-800 text-sm flex items-center"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 ml-1" viewBox="0 0 20 20" fill="currentColor">
                          <path fillRule="evenodd" d="M12.316 3.051a1 1 0 01.633 1.265l-4 12a1 1 0 11-1.898-.632l4-12a1 1 0 011.265-.633zM5.707 6.293a1 1 0 010 1.414L3.414 10l2.293 2.293a1 1 0 11-1.414 1.414l-3-3a1 1 0 010-1.414l3-3a1 1 0 011.414 0zm8.586 0a1 1 0 011.414 0l3 3a1 1 0 010 1.414l-3 3a1 1 0 11-1.414-1.414L16.586 10l-2.293-2.293a1 1 0 010-1.414z" clipRule="evenodd" />
                        </svg>
                        {showCommentCodeInput === post.id ? 'إخفاء الكود' : 'إضافة كود'}
                      </button>
                    </div>
                    <button
                      type="button"
                    ...
                    >
                      {isSubmittingComment ? 'جارِ الإرسال...' : 'إرسال'}
                    </button>
                  </div>
                </div>
              </div>
